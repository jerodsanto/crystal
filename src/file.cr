...
class File < IO::FileDescriptor
  ...
  def initialize(filename, mode = "r", perm = DEFAULT_CREATE_MODE, encoding = nil)
  ...
  def self.open(filename, mode = "r", perm = DEFAULT_CREATE_MODE, encoding = nil)
    new filename, mode, perm, encoding
  end

  def self.open(filename, mode = "r", perm = DEFAULT_CREATE_MODE, encoding = nil)
    file = File.new filename, mode, perm, encoding
    begin
      yield file
    ensure
      file.close
  ...
  # # => foo
  # ```
  def self.read(filename, encoding = nil)
  ...
  # end
  # ```
  def self.each_line(filename, encoding = nil)
  ...
  # # => ["foo\n","bar\n"]
  # ```
  def self.read_lines(filename, encoding = nil)
    lines = [] of String
    each_line(filename, encoding: encoding) do |line|
      lines << line
    end
    lines
  end
  ...
  # File.write("./foo", "bar")
  # ```
  def self.write(filename, content, perm = DEFAULT_CREATE_MODE, encoding = nil)
